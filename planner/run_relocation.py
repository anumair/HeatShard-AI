...
def main():
    ...
    record_load = record_recent_load(args.db, window_lookback=args.window_lookback)
    ...
    window_start = args.window_start or pick_target_window(args.db, args.min_probability)
    if window_start is None:
        print("\nno predictions above the probability floor -- run predictor/run_prediction.py first")
        return
    ...
    prediction_rows = conn.execute(
        ...
        (window_start, args.min_probability),
    ).fetchall()
    conn.close()

    print(f"\nplanning from window_start={window_start} ({len(prediction_rows)} candidate(s) above p >= {args.min_probability})")
    ...
    planner_storage.write_plan(args.db, window_start, moves)
    ...
    baseline = whole_shard_migration_volume(record_load, hot_record_ids, cluster)
